...
@Component({
  ...
  standalone: true,
  imports: [ShareModule],
  templateUrl: './pm013.component.html',
  styleUrl: './pm013.component.scss',
})
export class PM013Component {
  constructor(
    ...
    this.globalService.setBreadcrumb([
      {
        name: 'Báo cáo thống kê nguyên nhân hư hỏng',
        path: 'report/PM013',
      },
    ])
  }
  ...
  exportData(type: string) {
    ...
    if (type == '1') {
      ...
      this.http
        .post(environment.baseApiUrl + `/print_exel/ZPG_PM010`, param)
        .subscribe({
          next: (response: any) => {
            HideLoading()
            var contentType = 'application/vnd.ms-excel'
            var theBlob = Base64ToBlob(response.erawData, contentType)
            SaveBlob(theBlob, 'PM013_BaoCaoThongKeNguyenNhanHuHong.xls')
          },
          error: (error) => {
            HideLoading()
      ...
      this.http
        .post(environment.baseApiUrl + `/print_pdf/ZPG_PM010`, param)
        .subscribe({
          next: (response: any) => {
            HideLoading()
            var contentType = 'application/vnd.ms-excel'
            var theBlob = EBase64ToBlob(response.erawData, contentType)
            SaveBlob(theBlob, 'PM013_BaoCaoThongKeNguyenNhanHuHong.pdf')
          },
          error: (error) => {
            HideLoading()
            console.error('Fail load:', error)
