...
@Component({
  selector: 'app-pm004-a',
  standalone: true,
  imports: [ShareModule],
  templateUrl: './pm004-a.component.html',
  styleUrl: './pm004-a.component.scss'
})
export class PM004AComponent {
  constructor(
  ...
  exportData() {
    ...
    this.http.post(environment.baseApiUrl + `/export_report/ZPG_PM_QM_INFO/PM004A`, param)
      .subscribe({
        next: (response: any) => {
          HideLoading()
          var contentType = "application/vnd.ms-excel";
          var theBlob = EBase64ToBlob(response.erawData, contentType);
          SaveBlob(theBlob, "PM004A-PhieuDeNghiSuaChua.xlsx");
        },
        error: (error) => {
          HideLoading();
