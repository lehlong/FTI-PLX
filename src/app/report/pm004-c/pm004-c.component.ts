...
@Component({
  selector: 'app-pm004-c',
  standalone: true,
  imports: [ShareModule],
  templateUrl: './pm004-c.component.html',
  styleUrl: './pm004-c.component.scss'
})
export class PM004CComponent {
  constructor(
  ...
  exportData() {
    ...
    this.http.post(environment.baseApiUrl + `/export_report/ZPG_PM_QM_INFO/PM004C`, param)
      .subscribe({
        next: (response: any) => {
          HideLoading()
          var contentType = "application/vnd.ms-excel";
          var theBlob = Base64ToBlob(response.erawData, contentType);
          SaveBlob(theBlob, "PM004C_BangKePhieuSuaChua.xls");
        },
        error: (error) => {
          HideLoading()
