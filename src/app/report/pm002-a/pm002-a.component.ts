...
@Component({
  selector: 'app-pm002-a',
  standalone: true,
  imports: [ShareModule],
  templateUrl: './pm002-a.component.html',
  styleUrl: './pm002-a.component.scss'
})
export class PM002AComponent {
  constructor(
    ...
    this.globalService.setBreadcrumb([
      {
        name: 'Danh mục hệ thống CSVCKT (Tổng hợp)',
        path: 'report/PM002A',
      },
    ])
  }
  ...
  exportData() {
    ...
    this.http.post(environment.baseApiUrl + `/export_report/ZPG_EQUI_CORE/PM002A`, param)
      .subscribe({
        next: (response: any) => {
          HideLoading()
          var contentType = "application/vnd.ms-excel";
          var theBlob = Base64ToBlob(response.erawData, contentType);
          SaveBlob(theBlob, "PM002A.xls");
        },
        error: (error) => {
          HideLoading();
