...
export class PreparingTemplateListTableComponent {
  ...
  getNameTemplate() {
    const mgCode = this.auditPeriodListTables.find(
      (tem: any) => tem.code === Number(this.code),
    )

    const mgListTablesCode = mgCode ? mgCode.listTablesCode : null

    const templateList = this.templateListTables.find(
      (tem: any) => tem.mgListTablesCode === mgListTablesCode,
    )
  ...
  importExcelAndUpdateData(file: File) {
    this._auditPeriodService.uploadExcel(file, this.code).subscribe({
      next: (result) => {
  ...
  async getAllListTables() {
    try {
      this.listTables = await this._ltbs.GetAll().toPromise()
    } catch (error) {
      console.log(error)
    }
  }
  ...
  handleReviewOk() {
    console.log('code', this.currentData.code)
    console.log('text', this.reviewOpinionText)
    this._auditPeriodService
  ...
  handleAuditNotesOk(): void {
    if (this.currentEditDataAudit) {
      ...
      console.log(this.currentEditDataAudit)
    }
