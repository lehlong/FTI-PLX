...
export class ListAuditComponent {
  ...
  //   this._service.getListAuditByCode(data.code).subscribe({
  //     next: (result) => {
  //       if (result.data && result.data.length > 0) {
  //         const ListAuditData = result.data[0]
  //         console.log(ListAuditData)
  ...
  openEdit(data: { code: string }) {
    this._service.getListAuditHistory(data.code).subscribe({
      next: (result) => {
        if (result) {
          console.log(result);  // Log toàn bộ kết quả trả về từ API
          this.router.navigate(['/master-data/list-audit-edit', data.code], {
