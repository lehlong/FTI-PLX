...
export class AuditPeriodListTablesComponent {
  ...
  constructor(
    ...
    private _mgListTablesService: MgListTablesService,
  ...
  handleOk(): void {
    // Lọc các bảng mới được chọn
    const newlySelectedTables = this.listOfTables.filter(
      (table) =>
        table.isChecked && !this.previouslyCheckedTables.includes(table.code),
    )
    ...
    newlySelectedTables.forEach((table) => {
      const params = {
        isActive: true,
        listTablesCode: table.code,
  ...
  loadTables(): void {
    this._service
      .GetMgListTablesByAuditPeriodCode(this.lstAuditCode)
      .subscribe((response) => {
  ...
  getAllListTables() {
    this.isSubmit = false
    this._mgListTablesService.getall().subscribe({
      next: (data) => {
        this.mgListTable = data
      },
      error: (response) => {
        console.log(response)
      },
  ...
  getMgListTablesName(listTablesCode: string) {
    return this.mgListTable.find(
